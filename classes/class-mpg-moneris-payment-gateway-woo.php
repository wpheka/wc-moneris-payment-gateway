...
class mpg_WOO_Moneris_Payment_Gateway extends WC_Payment_Gateway_CC {
						    ...
						    <div class="inside">
						        <div class="support-widget">
						            ...
						            <ul>
						                ...
						                <li>» <a href="https://wpheka.com/submit-ticket/" target="_blank">Support Request</a></li>
						                <li>» <a href="https://wpheka.com/product/wc-moneris-payment-gateway/" target="_blank">Documentation and Common issues.</a></li>
						                <li>» <a href="https://wpheka.com/plugins/" target="_blank">Our Plugins Shop</a></li>
						            </ul>
						        </div>
						    </div>
						</div>
	...
	public function init_form_fields() {
		...
		$this->form_fields = array(
			...
			'preferred_cards' => array(
				'title'		=> __( 'Preferred Cards', $MPG_Moneris_Payment_Gateway->text_domain ),
				'type'		=> 'multiselect',
				'class'     => 'wc-enhanced-select',
				'desc_tip'	=> __( 'Select your desired cards from the multiple-select box. The logo of the selected card(s) will be displayed on the checkout page.', $MPG_Moneris_Payment_Gateway->text_domain ),
				'default'     => 'visa',
				'options' => array(
					'visa' => __( 'Visa', $MPG_Moneris_Payment_Gateway->text_domain ),
					'mastercard' => __( 'MasterCard', $MPG_Moneris_Payment_Gateway->text_domain ),
					'discover' => __( 'Discover', $MPG_Moneris_Payment_Gateway->text_domain ),
					'amex' => __( 'American Express', $MPG_Moneris_Payment_Gateway->text_domain ),
					'jcb' => __( 'JCB', $MPG_Moneris_Payment_Gateway->text_domain )
				),
				'custom_attributes' => array(
					'data-placeholder' => __( 'Select your desired cards', $MPG_Moneris_Payment_Gateway->text_domain ),
				),
			),
			'sandbox' => array(
	...
	public function validate_fields() {
		...
		if ( empty( $posted_data[$this->id.'-card-number'] ) ) {
			wc_add_notice( __('Please enter your card number.', $MPG_Moneris_Payment_Gateway->text_domain), 'error' );
			return false;
		}

		if ( empty( $posted_data[$this->id.'-card-expiry'] ) ) {
			wc_add_notice( __('Please enter your card expiry.', $MPG_Moneris_Payment_Gateway->text_domain), 'error' );
			return false;
		}

		if ( empty( $posted_data[$this->id.'-card-cvc'] ) ) {
			wc_add_notice( __('Please enter your card cvd code.', $MPG_Moneris_Payment_Gateway->text_domain), 'error' );
			return false;
		}
	...
	public function process_payment($order_id) {
		...
		if ( empty($store_id) || empty($api_token) ) {
			throw new Exception( __( 'Please update your Moneris credentials in payment settings.', $MPG_Moneris_Payment_Gateway->text_domain ) );
		}
