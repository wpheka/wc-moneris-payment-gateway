...
class WPHEKA_Gateway_Moneris extends WC_Payment_Gateway_CC {
	...
	public function process_payment( $order_id ) {
		...
		if ( $this->transaction_success( $response ) ) {
			...
			add_post_meta( $order_id, '_transaction_id', $response->getTxnNumber(), true );
			add_post_meta( $order_id, '_completed_date', $_date, true );
			add_post_meta( $order_id, '_reference_no', $response->getReferenceNum(), true );
			add_post_meta( $order_id, '_response_code', $response->getResponseCode(), true );
			add_post_meta( $order_id, '_iso_code', $response->getISO(), true );
			add_post_meta( $order_id, '_authorization_code', $response->getAuthCode(), true );
			add_post_meta( $order_id, '_transaction_type', $response->getTransType(), true );
