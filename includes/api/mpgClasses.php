...
class httpsPost {
	...
	public function __construct( $url, $dataToSend ) {
		...
		if ( $this->debug == true ) {
			WPHEKA_Moneris_Logger::log( 'DataToSend= ' . $this->dataToSend );
			WPHEKA_Moneris_Logger::log( 'PostURL= ' . $this->url );
		}
		...
		if ( $this->debug == true ) {
			WPHEKA_Moneris_Logger::log( 'RESPONSE=' . $this->response );
		}
	}
	...
	public function __construct( $storeid, $apitoken, $mpgRequestOBJ ) {
		...
		if ( ! $response ) {
				...
				$response = '<?xml version="1.0"?><response><receipt>' .
					...
					'<Message>Global Error Receipt - Error connecting to moneris servers. This can be caused by a firewall or your internet connection. Also make sure that you support TLS 1.1 or 1.2.</Message><TransAmount>null</TransAmount>' .
